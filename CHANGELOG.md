...
- **Multiline CSS Value Normalization**: Multi-line CSS declarations (e.g., shadows split across lines) are now collapsed to single-line values ([#16](https://github.com/0xstern/tailwind-resolver/issues/16))
  - `theme.ts` no longer contains literal `\n` characters within CSS values
  - Whitespace sequences (newlines, tabs, multiple spaces) are normalized to a single space during extraction
- **Nested Non-Color Properties in Type Generation**: Properties like shadows and animations with dashed keys (e.g., `--shadow-skeuomorphic-xs`, `--animate-circular-progress`) now generate correct nested types instead of `[object Object]` ([#16](https://github.com/0xstern/tailwind-resolver/issues/16))
  - `generateRecordType` and `generateMixedRecordType` now recursively handle nested objects using the `isRecord` type guard
  - Applies to all namespaces: shadows, animations, blur, radius, ease, etc.
